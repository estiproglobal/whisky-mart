...
- **Increment 12C "Repair Pass": ✅ CODE COMPLETE on `increment-12c-repair`, PR open (NOT merged; the 12C prompt says wait for owner review of the Vercel preview).** Repaired 12B's broken visual layer. **Part 1 diagnosis:** the atmosphere-photo binaries were never committed (only `CREDITS.md` was tracked) and the image CDNs stay 403-blocked from the build box, so every `<Photo>` slot fell back to near-black and read as empty; the product SVG renders and header icons were actually fine. **Part 2 (five defects):** header icons already correct; numeric PDP "House rating" removed; "Members £..." removed (data left dormant); label-plate region/cask formatted via new `formatRegion`/`formatCask` ("islay"→"Islay", "ex-bourbon"→"Ex-bourbon cask"); all four guides deepened to 708–744 words (4-min reads) with grounded embeds. **Part 3:** warm `.atmosphere-fallback` so photo-absent slots look intentional; conditional hero scrim; remove-one-accessory = journal cover hover-dim cut. **Gates:** typecheck/lint ✓ · 106 tests ✓ · build 102 pages ✓ · copy audit clean · Lighthouse home 93/96, PDP 91/97 · desktop + 390px checked.
  - ⚠️ **OWNER-GATED, the one open item:** real atmosphere photography. The sandbox cannot reach Unsplash/Pexels (403). The PR ships the designed fallback. To land real photos: allowlist the image CDNs in the environment network settings, then run `apps/web/scripts/fetch-photos.mjs` (8 slots pre-curated; `still` + `islay-coast` need a pick) and commit; `<Photo>` already keeps `priority` preload for the hero LCP. This is the only unmet part of the 12C Part 1 acceptance and is not resolvable from the build environment.
