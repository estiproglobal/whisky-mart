    ...
    <div className="space-y-6">
      {blocks.map((block, i) => {
        switch (block.kind) {
          case "heading":
            return (
              <h2 key={i} className="font-display text-2xl text-charcoal">
                {block.text}
              </h2>
            );
          case "paragraph":
          ...
          case "quote":
            return (
              <blockquote key={i} className="border-l-4 border-whisky-300 pl-5 font-display text-xl text-charcoal/80">
                “{block.text}”
                {block.cite ? <footer className="mt-1 text-sm text-charcoal/50">— {block.cite}</footer> : null}
              </blockquote>
            );
          case "products": {
